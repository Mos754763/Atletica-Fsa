...
export function Storefront({ products }: StorefrontProps) {
  ...
  return <main className="store-page">
    ...
    <nav className="store-filters" aria-label="Filtrar produtos por categoria"><button className={selectedCategory === "todos" ? "is-active" : ""} type="button" onClick={() => setSelectedCategory("todos")}>Todos</button>{categories.map((category) => <button className={selectedCategory === category.slug ? "is-active" : ""} type="button" key={category.id} onClick={() => setSelectedCategory(category.slug)}>{category.name}</button>)}</nav>
