...
export function Storefront({ products }: StorefrontProps) {
  ...
  useEffect(() => {
    if (isCartOpen) {
      cartWasOpenRef.current = true;
      cartCloseButtonRef.current?.focus();
      return;
    }
    if (cartWasOpenRef.current) {
      cartTriggerRef.current?.focus();
      cartWasOpenRef.current = false;
    }
  }, [isCartOpen]);
  ...
  return <main className="store-page">
    <header className="store-nav">
      ...
      <nav aria-label="Navegação da loja"><Link className="store-nav__events" href="/eventos">Eventos</Link><button ref={cartTriggerRef} type="button" className="store-cart-trigger" onClick={() => setCartOpen(true)}><ShoppingBag size={17} /> Carrinho {totalQuantity > 0 && <b>{totalQuantity}</b>}</button></nav>
    </header>
    ...
    <section className="store-grid" aria-live="polite">
      {visibleProducts.map((product, index) => {
        ...
        return <article className="store-product store-product--interactive fx-spotlight" key={product.id} onPointerMove={(event) => { if (event.pointerType !== "mouse") return; const bounds = event.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(100, ((event.clientX - bounds.left) / bounds.width) * 100)); const y = Math.max(0, Math.min(100, ((event.clientY - bounds.top) / bounds.height) * 100)); event.currentTarget.style.setProperty("--store-card-x", `${x}%`); event.currentTarget.style.setProperty("--store-card-y", `${y}%`); event.currentTarget.style.setProperty("--store-card-tilt-x", `${((y - 50) / 50) * -2.1}deg`); event.currentTarget.style.setProperty("--store-card-tilt-y", `${((x - 50) / 50) * 2.1}deg`); }} onPointerLeave={(event) => { event.currentTarget.style.removeProperty("--store-card-tilt-x"); event.currentTarget.style.removeProperty("--store-card-tilt-y"); }}>
          <button type="button" className={`store-product__visual store-product__visual--${index % 6}`} onClick={() => setSelectedProduct(product)} aria-label={`${product.isFeatured ? "Destaque. " : ""}${resolveFsaProductImage(product.name, product.imageUrl) ? "" : "FSA. "}Ver produto: ${product.name}`}>
            {resolveFsaProductImage(product.name, product.imageUrl) ? <img src={resolveFsaProductImage(product.name, product.imageUrl)!} alt={product.name} loading={index < 4 ? "eager" : "lazy"} decoding="async" /> : <span aria-hidden="true">FSA</span>}{product.isFeatured && <small>DESTAQUE</small>}<i><Eye size={17} /> Ver produto</i>
          </button>
