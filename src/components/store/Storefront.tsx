        ...
        {visibleProducts.map((product, index) => (
          <article className="store-product" key={product.id}>
            <div className={`store-product__visual store-product__visual--${index % 6}`}>
              {resolveFsaProductImage(product.name, product.imageUrl) ? <Image src={resolveFsaProductImage(product.name, product.imageUrl)!} alt={product.name} fill sizes="(max-width: 760px) 50vw, 25vw" /> : <span>FSA</span>}
