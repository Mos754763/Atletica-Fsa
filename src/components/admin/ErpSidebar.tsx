  ...
  return (
    <aside className={`erp-sidebar${collapsed ? " is-collapsed" : ""}`}>
      <motion.div className="erp-sidebar__top" initial={false} animate={{ opacity: 1, x: 0 }} transition={reducedMotion ? { duration: 0 } : { ...MOTION_SPRING, delay: 0.04 }}>
        ...
        <button className="erp-sidebar__collapse" type="button" onClick={() => onCollapsedChange(!collapsed)} aria-controls={navigationId} aria-expanded={!collapsed} aria-label={collapsed ? "Expandir a navegação do ERP" : "Recolher a navegação do ERP"} title={collapsed ? "Expandir navegação" : "Recolher navegação"}>{collapsed ? <PanelLeftOpen size={17} /> : <PanelLeftClose size={17} />}</button>
        <div className="erp-sidebar__context"><span>ERP FSA</span><strong>Operação</strong></div>
      </motion.div>

      <motion.nav id={navigationId} className="erp-sidebar__nav" aria-label="Navegação do ERP" initial={false} animate={{ opacity: 1, x: 0 }} transition={reducedMotion ? { duration: 0 } : { ...MOTION_SPRING, delay: 0.1 }}>
        <span className="erp-sidebar__label">OPERAÇÃO</span>
        {availableItems.map(({ href, label, icon: Icon }) => { const isActive = getItemClassName(href) === "is-active"; return <Link href={href} className={getItemClassName(href)} aria-current={isActive ? "page" : undefined} data-tooltip={collapsed ? label : undefined} title={collapsed ? label : undefined} key={href}><Icon size={17} /><span>{label}</span>{isActive && <ChevronRight size={15} />}</Link>; })}
