  ...
  return (
    <aside className={`erp-sidebar${collapsed ? " is-collapsed" : ""}`}>
      ...
      <motion.nav className="erp-sidebar__nav" aria-label="Navegação do ERP" initial={false} animate={{ opacity: 1, x: 0 }} transition={reducedMotion ? { duration: 0 } : { ...MOTION_SPRING, delay: 0.1 }}>
        ...
        {isPresident && <Link href="/admin/atividades" className={getItemClassName("/admin/atividades")} aria-current={pathname.startsWith("/admin/atividades") ? "page" : undefined} title={collapsed ? "Atividades" : undefined}><ScrollText size={17} /><span>Atividades</span>{pathname.startsWith("/admin/atividades") && <ChevronRight size={15} />}</Link>}
