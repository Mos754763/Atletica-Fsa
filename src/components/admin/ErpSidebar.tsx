...
import { BarChart3, Bot, Boxes, Building2, CalendarDays, ChevronLeft, ChevronRight, LayoutDashboard, ListChecks, PanelLeftClose, PanelLeftOpen, ShoppingBag, Table2, UsersRound } from "lucide-react";
  ...
  return (
    <aside className={`erp-sidebar${collapsed ? " is-collapsed" : ""}`}>
      <motion.div className="erp-sidebar__top" initial={false} animate={{ opacity: 1, x: 0 }} transition={reducedMotion ? { duration: 0 } : { ...MOTION_SPRING, delay: 0.04 }}>
        <Link href="/admin" className="erp-sidebar__brand" aria-label="ERP ATLETICA FSA - visão geral">
          <FsaWordmark />
          <span className="erp-sidebar__brand-mark" aria-hidden="true">FSA</span>
        </Link>
        <button className="erp-sidebar__collapse" type="button" onClick={() => onCollapsedChange(!collapsed)} aria-expanded={!collapsed} aria-label={collapsed ? "Expandir a navegação do ERP" : "Recolher a navegação do ERP"} title={collapsed ? "Expandir navegação" : "Recolher navegação"}>
      ...
      <motion.nav className="erp-sidebar__nav" aria-label="Navegação do ERP" initial={false} animate={{ opacity: 1, x: 0 }} transition={reducedMotion ? { duration: 0 } : { ...MOTION_SPRING, delay: 0.1 }}>
        ...
        {availableItems.map(({ href, label, icon: Icon }) => {
          const isActive = getItemClassName(href) === "is-active";
          return <Link href={href} className={getItemClassName(href)} aria-current={isActive ? "page" : undefined} title={collapsed ? label : undefined} key={href}><Icon size={17} /><span>{label}</span>{isActive && <ChevronRight size={15} />}</Link>;
        })}
      ...
      <div className="erp-sidebar__footer">
        <Link href="/conta" className="erp-sidebar__account" title={collapsed ? "Minha conta" : undefined}><span className="erp-sidebar__avatar">{displayName.slice(0, 1).toUpperCase()}</span><span className="erp-sidebar__account-copy"><strong>{displayName}</strong><small>{roleLabel(role)}</small></span></Link>
        <SignOutButton />
      </div>
    </aside>
  );
