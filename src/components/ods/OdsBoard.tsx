...
import { ArrowLeft, ClipboardList, Clock3, PackageCheck, Plus, RefreshCw, Trash2, X } from "lucide-react";
...
const nextStatus: Partial<Record<OrderState, OrderState>> = { aguardando_pagamento: "pago", pago: "em_preparo", em_preparo: "pronto", pronto: "entregue" };
const nextLabel: Partial<Record<OrderState, string>> = { aguardando_pagamento: "Confirmar pagamento", pago: "Assumir preparo", em_preparo: "Marcar pronto", pronto: "Entregar pedido" };
...
export function OdsBoard({ catalog }: { catalog: CatalogProduct[] }) {
  ...
  const [manualPaymentStatus, setManualPaymentStatus] = useState<"aprovado" | "pendente">("aprovado");
  ...
  async function createManualOrder(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/ods/orders", { method: "POST", headers: { Authorization: `Bearer ${await getToken()}`, "Content-Type": "application/json" }, body: JSON.stringify({ fulfillment: manualFulfillment, paymentMethod: manualPaymentMethod, paymentStatus: manualPaymentStatus, customerName: manualCustomerName, notes: manualNotes, items: manualCart.map((item) => ({ productId: item.productId, variantId: item.variantId, quantity: item.quantity })) }) });
  ...
  return <section className="ods-board" aria-busy={loading || Boolean(updatingId)}>
    <header><div><p>GESTÃO OPERACIONAL EM TEMPO REAL</p><h2>Pedidos ativos</h2></div><div className="ods-board__actions"><Link className="ods-back-link" href="/admin/pedidos"><ArrowLeft size={16} /> Pedidos no ERP</Link><button className="ods-new-order" type="button" onClick={() => setShowManualForm((current) => !current)} aria-controls="ods-manual-order" aria-expanded={showManualForm}><Plus size={16} /> Novo pedido</button><button type="button" onClick={() => void loadOrders()} disabled={loading} aria-busy={loading}><RefreshCw size={16} className={loading ? "is-spinning" : ""} /> Atualizar</button></div></header>
    {error && <p className="ods-error" role="alert">{error}</p>}
    {showManualForm && <form id="ods-manual-order" className="ods-manual-order" onSubmit={createManualOrder} aria-busy={creatingManualOrder}><div className="ods-manual-order__heading"><div><p>NOVO PEDIDO OPERACIONAL</p><h3>{manualPaymentStatus === "pendente" ? "Registrar encomenda com pagamento na retirada." : "Registrar pedido no balcão."}</h3></div><button className="ods-manual-order__close" type="button" onClick={() => setShowManualForm(false)} aria-label="Fechar novo pedido"><X size={18} /></button></div><div className="ods-manual-order__fields"><label>Cliente (opcional)<input value={manualCustomerName} onChange={(event) => setManualCustomerName(event.target.value)} maxLength={120} placeholder="Nome para identificação" /></label><label>Atendimento<select value={manualFulfillment} onChange={(event) => setManualFulfillment(event.target.value as "retirada" | "consumo_local")}><option value="retirada">Retirada</option><option value="consumo_local">Consumo no local</option></select></label><label>Pagamento<select value={manualPaymentStatus} onChange={(event) => setManualPaymentStatus(event.target.value as "aprovado" | "pendente")}><option value="aprovado">Já recebido</option><option value="pendente">Pagar na retirada</option></select></label><label>Forma prevista<select value={manualPaymentMethod} onChange={(event) => setManualPaymentMethod(event.target.value as "pix_presencial" | "dinheiro")}><option value="pix_presencial">PIX presencial</option><option value="dinheiro">Dinheiro</option></select></label></div><div className="ods-manual-order__add"><select value={selectedCatalogItem} onChange={(event) => setSelectedCatalogItem(event.target.value)} aria-label="Selecione um item do catálogo"><option value="">Selecione produto ou variação disponível</option>{catalogItems.map((item) => <option key={item.key} value={item.key}>{item.label} · {formatBrl(item.priceCents)} · {item.stockQuantity} em estoque</option>)}</select><button type="button" onClick={addManualItem} disabled={!selectedCatalogItem}>Adicionar</button></div><div className="ods-manual-order__cart">{manualCart.length === 0 && <p>Nenhum item selecionado. Escolha um produto do catálogo acima.</p>}{manualCart.map((item) => <div className="ods-manual-order__item" key={item.key}><span><strong>{item.label}</strong><small>{formatBrl(item.priceCents)} cada</small></span><label>Qtd.<input type="number" min="1" max={item.stockQuantity} value={item.quantity} onChange={(event) => setManualCart((current) => current.map((cartItem) => cartItem.key === item.key ? { ...cartItem, quantity: Math.max(1, Math.min(Number(event.target.value) || 1, cartItem.stockQuantity)) } : cartItem))} /></label><b>{formatBrl(item.priceCents * item.quantity)}</b><button type="button" onClick={() => setManualCart((current) => current.filter((cartItem) => cartItem.key !== item.key))} aria-label={`Remover ${item.label}`}><Trash2 size={16} /></button></div>)}</div><label className="ods-manual-order__notes">Observações (opcional)<textarea value={manualNotes} onChange={(event) => setManualNotes(event.target.value)} maxLength={500} rows={2} placeholder="Ex.: retirar no intervalo" /></label><footer><span>Total do pedido <strong>{formatBrl(manualTotalCents)}</strong></span><button type="submit" disabled={creatingManualOrder || manualCart.length === 0}>{creatingManualOrder ? "Registrando..." : manualPaymentStatus === "pendente" ? "Salvar encomenda pendente" : "Confirmar pedido pago"}</button></footer></form>}
    ...
    {!loading && orders.length === 0 && <div className="ods-empty"><ClipboardList size={34} /><strong>Nenhum pedido em processamento.</strong><span><Clock3 size={16} /> Novos pedidos confirmados aparecerão aqui automaticamente.</span></div>}
    {!loading && <div className="ods-board__grid">{orders.map((order) => <article className={`ods-order fx-spotlight ods-order--${order.status}`} key={order.id}><header><div><span>#{order.order_number}</span><strong aria-live="polite">{ORDER_STATUS_LABEL[order.status]}</strong></div><time>{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(order.created_at))}</time></header><h3>{order.customer_name || "Pedido FSA"}</h3><ul>{order.order_items.map((item, index) => <li key={`${item.product_name}-${index}`}><b>{item.quantity}×</b> {item.product_name}</li>)}</ul><footer><span>{order.fulfillment === "consumo_local" ? "Consumo no local" : "Retirada"}</span>{order.status === "pronto" && order.fulfillment === "retirada" && <label className="ods-pickup-code">QR Code de retirada<input value={pickupTokens[order.id] ?? ""} onChange={(event) => setPickupTokens((current) => ({ ...current, [order.id]: event.target.value }))} inputMode="text" autoComplete="off" maxLength={64} placeholder="Escaneie o QR do cliente" /></label>}{nextStatus[order.status] && <button type="button" disabled={updatingId === order.id} aria-busy={updatingId === order.id} onClick={() => void advanceOrder(order)}>{updatingId === order.id ? "Atualizando..." : <><PackageCheck size={16} /> {nextLabel[order.status]}</>}</button>}</footer></article>)}</div>}
  </section>;
}
