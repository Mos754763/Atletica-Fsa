...
type CustomTableOption = { id: string; sector_id: string; name: string };
...
export default async function OrganizationAdminPage() {
  ...
  const [{ data: sectors }, { data: profiles }, { data: memberships }, { data: grants }, { data: customTables }] = await Promise.all([
    ...
    service.from("custom_tables").select("id,sector_id,name").is("deleted_at", null).order("name").returns<CustomTableOption[]>(),
  ]);
  ...
  return <main className="members-page">
    ...
    <section className="members-layout" style={{ marginTop: 20 }}>
      ...
      <section className="members-list-card"><div className="members-list-card__head"><div><span>PERMISSÕES EXPLÍCITAS</span><h2>Concessões especiais</h2></div><span>{(grants ?? []).length} ativas</span></div><form action={grantSectorPermission} className="members-form" style={{ padding: 25, borderBottom: "1px solid var(--line)" }}><label>Integrante<select name="profileId" required>{(profiles ?? []).map((profile) => <option key={profile.id} value={profile.id}>{profileName(profile)}</option>)}</select></label><label>Setor<select name="sectorId" required>{activeSectors.map((sector) => <option key={sector.id} value={sector.id}>{sector.name}</option>)}</select></label><label>Escopo da tabela<select name="resourceKey" defaultValue="table:*" required><option value="table:*">Todas as tabelas do setor</option>{(customTables ?? []).map((table) => <option key={table.id} value={`table:${table.id}`}>{table.name} · tabela específica</option>)}</select></label><label>Ação<select name="action" defaultValue="ver">{PERMISSION_ACTIONS.map((action) => <option key={action} value={action}>{action}</option>)}</select></label><button type="submit">Conceder acesso</button></form><div className="members-table-wrap"><table><thead><tr><th>Pessoa</th><th>Escopo</th><th>Ação</th><th></th></tr></thead><tbody>{(grants ?? []).map((grant) => <tr key={grant.id}><td><strong>{profileName(grant.profile)}</strong></td><td><small>{grant.sector?.name || "Escopo legado"} · {grant.resource_key}</small></td><td><span className="member-role member-role--caixa">{grant.action}</span></td><td><form action={revokeSectorPermission}><input type="hidden" name="grantId" value={grant.id} /><button type="submit">Revogar</button></form></td></tr>)}{(grants ?? []).length === 0 && <tr><td colSpan={4} className="members-empty">Nenhuma permissão especial ativa.</td></tr>}</tbody></table></div></section>
    </section>
  </main>;
}
