  ...
  return <main className="cms-page">
    ...
    <section className="cms-layout"><div className="cms-forms">
      <article className="cms-card"><div className="cms-card__title"><Plus size={18} /><h2>Novo produto</h2></div><form action={createProduct} className="cms-form" encType="multipart/form-data">
        ...
        <label>Enviar foto (JPG, PNG ou WEBP — até 3 MB)<input name="imageFile" type="file" accept="image/jpeg,image/png,image/webp" /></label>
      ...
      <article className="cms-card"><div className="cms-card__title"><Plus size={18} /><h2>Variações de produto</h2></div><form action={createVariant} className="cms-form"><label>Produto-base<select name="productId" required defaultValue=""><option value="" disabled>Selecione um produto</option>{productList.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><div className="cms-form__split"><label>Cor<input name="color" placeholder="Ex.: Azul-marinho" /></label><label>Tamanho<input name="size" placeholder="Ex.: M" /></label></div><div className="cms-form__split"><label>Nome complementar (opcional)<input name="name" placeholder="Ex.: Edição atletas" /></label><label>SKU<input name="sku" placeholder="FSA-CAM-AZM" /></label></div><div className="cms-form__split"><label>Preço (R$, opcional)<input name="priceBrl" min="0" step="0.01" inputMode="decimal" type="text" placeholder="Usar preço do produto" /></label><label>Estoque inicial<input name="stockQuantity" required min="0" type="number" defaultValue="0" /></label></div><label>Observações comerciais (opcional)<input name="notes" placeholder="Ex.: modelagem unissex, algodão" /></label><p>Cadastre cor e tamanho no mesmo produto-base. A loja exigirá a escolha da variação antes de adicionar ao carrinho.</p><button type="submit">Salvar variação</button></form></article>
