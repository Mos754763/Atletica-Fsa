...
export async function registerForEvent(formData: FormData) {
  ...
  const { error } = await supabase.from("event_registrations").insert({ event_id: eventId, customer_id: userId, attendee_name: profile?.display_name || profile?.email || "Torcida FSA", attendee_email: profile?.email || null, amount_cents: event.registration_price_cents, status: event.registration_price_cents > 0 ? "pendente" : "confirmada" });
