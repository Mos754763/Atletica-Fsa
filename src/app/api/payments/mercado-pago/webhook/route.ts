...
import { verifyMercadoPagoWebhook } from "@/lib/payments/mercado-pago";
...
export async function POST(request: Request) {
  ...
  const supabase = createServiceClient(); const { data: order } = await supabase.from("orders").select("id,order_number,status,customer_id,customer_email").eq("id", orderId).single(); if (!order) return NextResponse.json({ ok: true, ignored: "unknown_order" });
