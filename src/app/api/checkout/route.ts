...
export async function POST(request: Request) {
  ...
  let items: Array<{ productId: string; quantity: number; product: { id: string; name: string; price_cents: number; stock_quantity: number; is_active: boolean } }>;
  try {
    items = parsed.data.items.map((item) => { const product = products.find((candidate) => candidate.id === item.productId)!; if (!product.is_active || product.stock_quantity < item.quantity) throw new Error(`Estoque indisponível para ${product.name}.`); return { ...item, product }; });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Estoque indisponível." }, { status: 400 });
  }
  ...
  try {
    ...
    if (orderId) {
      await supabase.from("orders").update({ status: "cancelado" }).eq("id", orderId).eq("status", "aguardando_pagamento");
      await supabase.from("order_status_history").insert({ order_id: orderId, status: "cancelado", changed_by: profile.id, note: "Falha ao iniciar o Checkout Mercado Pago" });
    }
