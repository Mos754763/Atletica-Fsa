...
export async function GET(request: Request) {
  ...
  const { data, error } = await auth.supabase.from("orders").select("id,order_number,status,fulfillment,customer_name,notes,created_at,pickup_code,order_items(product_name,quantity)").in("status", visibleStates).order("created_at", { ascending: true });
...
export async function PATCH(request: Request) {
  const auth = await getApiProfile(request); if ("error" in auth) return auth.error;
  if (!["admin", "cozinha"].includes(auth.profile.role)) return NextResponse.json({ error: "Acesso restrito à operação." }, { status: 403 });
  const body = await request.json().catch(() => null) as { orderId?: string; status?: OrderState; pickupCode?: string } | null; if (!body?.orderId || !body.status) return NextResponse.json({ error: "Atualização inválida." }, { status: 400 });
  ...
  if (body.status === "entregue" && order.fulfillment === "retirada" && (!body.pickupCode || body.pickupCode.trim().toUpperCase() !== order.pickup_code)) return NextResponse.json({ error: "Código de retirada inválido." }, { status: 422 });
  const { data: updatedOrder, error } = await auth.supabase.from("orders").update({ status: body.status, ready_at: body.status === "pronto" ? new Date().toISOString() : null, picked_up_at: body.status === "entregue" ? new Date().toISOString() : null, picked_up_by: body.status === "entregue" ? auth.profile.id : null }).eq("id", order.id).eq("status", order.status).select("id").maybeSingle(); if (error) return NextResponse.json({ error: "Não foi possível atualizar o pedido." }, { status: 500 });
