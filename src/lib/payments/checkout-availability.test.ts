...
describe("disponibilidade do checkout comercial", () => {
  it("bloqueia pagamento mesmo com token enquanto o gate comercial não estiver liberado", () => {
    expect(getCheckoutAvailability({ paymentsEnabled: false, mercadoPagoAccessToken: "APP_USR-token" })).toMatchObject({
      available: false,
  ...
  it("bloqueia pagamento quando a ativação existir sem token do provedor", () => {
    expect(getCheckoutAvailability({ paymentsEnabled: true })).toMatchObject({
      available: false,
  ...
  it("libera somente quando o gate e a credencial estiverem configurados", () => {
    expect(getCheckoutAvailability({ paymentsEnabled: true, mercadoPagoAccessToken: "APP_USR-token" })).toEqual({ available: true });
  });
});
