...
describe("controle de acesso por papel", () => {
  it("restringe áreas operacionais aos papéis permitidos", () => {
    ...
    expect(canAccessRole("backoffice", ["admin", "backoffice"])).toBe(true);
    expect(canAccessRole("caixa", ["admin", "backoffice"])).toBe(false);
  ...
  it("traduz os papéis para rótulos de interface", () => {
    ...
    expect(roleLabel("backoffice")).toBe("Backoffice");
