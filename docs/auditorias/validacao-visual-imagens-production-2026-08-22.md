...
A verificação no deployment de produção `dpl_FCJiTDqhuWZDwY3N5zhwuQLJJUh8` (commit `521b831`) repetiu o resultado nas mesmas larguras. Em **390 × 844 px**, a frase institucional permanece à esquerda e o coelho está ancorado abaixo e à direita, sem contato com a cópia. Em **360 × 800 px**, o texto permanece integralmente legível, o mascote não invade a coluna e o início do cartão de acesso continua alinhado à sequência da página. As duas capturas não apresentaram corte horizontal ou sobreposição entre controles e conteúdo.

## Verificação do acesso Google no layout compacto

Em produção, o botão **“Continuar com Google”** foi acionado em `https://atleticafsa.site/login?next=%2Fconta`. A autenticação retornou à aplicação e encaminhou a sessão para `/auth/mfa?next=%2Fconta`, onde a tela solicitou o código de seis dígitos do autenticador. Esse resultado confirma que o botão do layout compacto ainda inicia o provedor Google, que o callback interno preserva o destino `/conta` e que a garantia de MFA continua sendo aplicada antes do acesso à conta. Nenhum código MFA foi informado durante a verificação.
